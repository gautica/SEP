...
void controller_Callback(const sensor_msgs::Joy::ConstPtr &msg) {
	...
	if (msg->buttons[8] == 1) {                      //Share button is pressed
        ...
        if (msg->buttons[12] == 1) {                      //Share button is pressed 
            ...
            local_status.player2_topview = camera_top_view;
...
void link_Callback(const gazebo_msgs::LinkStates::ConstPtr &msg) {
   ...
   for(int i  = 0; i < msg->name.size(); i++) {
        ...
        if (msg->name[i].find("robot_1::base_footprint") != std::string::npos) { // get the base link pose of robot 
        ...
        {   
            ...
            new_resource.distance_to_gripper = sqrt( pow(msg->pose[i].position.x - robot_gripper_1_x, 2) + pow(msg->pose[i].position.y - robot_gripper_1_y, 2) ); // save distance to gripper of robot
...
int grab() {
    if (nearest_resource.distance_to_gripper < 0.1 && grabbed == false ) { // if its close enough to gripper
        if (sqrt( pow(nearest_resource.x - robot_gripper_0_x, 2) + pow(nearest_resource.y - robot_gripper_0_y, 2)) > 0.05) { // and not in the other gripper
            ROS_ERROR("case");
	    gazebo_msgs::LinkState set_resource;
            ...
            set_resource.pose.position.x = robot_gripper_1_x;
            set_resource.pose.position.y = robot_gripper_1_y;
	    ...
	    //Get Model-Name
            ...
            attach_srv.request.model_name_2 = "robot_1";
...
int drop() {
    if (nearest_resource.distance_to_gripper > 0 && nearest_resource.distance_to_gripper < 0.05 && grabbed == true) {
        ...
        resource_msg.roboter_name = "robot_1";
        ...
        dettach_srv.request.model_name_2 = "robot_1";
        ...
        gazebo_msgs::LinkState set_resource;
        set_resource.link_name = nearest_resource.name;
        set_resource.pose.position.x = (robot_gripper_1_x - robot_base_x)*2 + robot_base_x;
        set_resource.pose.position.y = (robot_gripper_1_y - robot_base_y)*2 + robot_base_y;
        set_resource.pose.position.z = 0.03;
        link_pose_publisher.publish(set_resource);
...
{
    init(argc, argv, "roboter_controller_2");
    NodeHandle n;
    Duration(0.01).sleep();
    controller = n.subscribe("/joy_2", 1, controller_Callback);
    status_sub = n.subscribe("/status", 1, status_Callback);
    link_sub = n.subscribe("/gazebo/link_states", 1, link_Callback);
    twist_publisher = n.advertise<geometry_msgs::Twist>("/robot_1/cmd_vel", 1);
